...
CapacityFactor["DE",11,"P_Wind_Offshore_Deep"]
...
@constraint(ESM, CapacityAccountingFunction[y in year, t in technologies, r in regions],
    sum(NewCapacity[yy,r,t] for yy in year if yy<=y && y-yy<= TechnologyLifetime[t])+ResidualCapacity[y,r,t]-CapacitySlack[y,r,t] == AccumulatedCapacity[y,r,t]
)
...
#@constraint(ESM, AdditionalityNationalAnnual[y in year],
 #   sum(FuelUseByTechnology[y, r, h, "Power2Gas", "Power"] for r in regions, h in hour) <=
   #     sum(OutputRatio[t, "Power"] * NewCapacity[y, r, t] * sum(CapacityFactor[r, h, t] for h in hour)
    #        for t in technologies, r in regions
     #       if TagRenewable[t] > 0 && TagDispatchableTechnology[t] == 0
      #  )
#)
